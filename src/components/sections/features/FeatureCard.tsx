      ...
      <div className={`relative w-full h-full rounded-[20px] ${isExpanded && 'rounded-[34px]'} bg-[#F8F8F805] flex flex-col overflow-hidden`}>
        ...
        <div className="flex-shrink-0 mt-2 relative">
          ...
          {isModal && onClose && (
            <button
              onClick={onClose}
              className="absolute top-3 right-5 flex-shrink-0 bg-white/10 hover:bg-[#FF838320] hover:border-[#FF838340] p-1.5 xs:p-2 sm:p-2 rounded-full border border-white/5 transition-all duration-200 hover:scale-110 hover:rotate-90"
              aria-label={`Close ${title} details`}
            >
              <X
                size={14}
                className="xs:w-4 xs:h-4 sm:w-4 sm:h-4 transition-all duration-200"
                color="#858FA660"
              />
            </button>
          )}
